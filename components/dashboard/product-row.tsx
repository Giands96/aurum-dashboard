...
export const ProductRow = React.memo(function ProductRow({
  ...
  const [{ menuTop, menuRight }, setMenuPos] = useState({
    menuTop: 0,
    menuRight: 0,
  });
  ...
  return (
    <tr className="bg-white hover:bg-gray-50/50 transition-colors">
      ...
      <td className="px-4 py-3">
        <StatusBadge status={product.activo ? "active" : "inactive"} />
      </td>
      <td className="px-4 py-3">
          ...
          {portalRef.current && menuOpen &&
            createPortal(
              <div
                ...
                className="w-44 rounded-xl border bg-white shadow-lg py-1"
              >
                <Link
  ...
  (prev, next) =>
    prev.product === next.product && prev.menuOpen === next.menuOpen,
);
